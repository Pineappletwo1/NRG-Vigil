...
var indexApp = (function () {
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(img, file$1, 16, 8, 438);
    			...
    			add_location(div4, file$1, 22, 4, 590);
    			attr_dev(div5, "class", "grid grid-rows-[1fr_60px] gap-4 h-full");
    			add_location(div5, file$1, 11, 0, 201);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = listen_dev(button, "click", /*changeProgress*/ ctx[1], false, false, false, false);
    				mounted = true;
    			}
    		},
    		p: function update(ctx, [dirty]) {
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	function changeProgress() {
    		$$invalidate(0, progress = 20);
    	}

    	const writable_props = [];

    	Object.keys($$props).forEach(key => {
    		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== '$$' && key !== 'slot') console.warn(`<ViewPage> was created with unknown prop '${key}'`);
    	});
